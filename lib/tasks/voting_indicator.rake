...
def update_selected_indicator(scorecard)
  scorecard.voting_indicators.each do |vi|
    raised_indi = scorecard.raised_indicators.select { |ri| ri.indicatorable_id == vi.indicatorable_id && ri.indicatorable_type == vi.indicatorable_type }.first
    raised_indi.update_column(:selected, true)
    vi.update_column(:raised_indicator_id, raised_indi.id)
  end
end
